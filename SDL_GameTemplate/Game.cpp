...
void Game::update() const
{
	...
	{
		mousepressedstart = mouse({ 344,232,311,60 }) ? true : false;
		if (mousepressedstart)
			startFlag = true;
		exitflag = mouse({ 392,370,216,60 }) ? true : false;
		if (exitflag)
			isRunning = 0;
	}
	...
	{
		SDL_Rect first{ 190,352,128,96 };
		SDL_Rect second{ 436,352,128,96 };
		SDL_Rect third{ 682,352,128,96 };

		Coordonate aux = mouse();

		if (aux.x > first.x && aux.x < first.x + first.w)
		{
	...
	{
		SDL_Rect lvl1{ 380,135,239,60 };
		SDL_Rect lvl2{ 374,295,251,60 };
		SDL_Rect lvl3{ 374,455,251,60 };

		Coordonate aux = mouse();

		if (aux.x > lvl1.x && aux.x < lvl1.x + lvl1.w)
		{
...
void Game::render() const
{
	if (mousepressedstart == true) //test daca iese din start
	{
		if (chooseCharacterflag != 0) //test daca a ales caracterul
		{
			if (mapChosen == true)//test daca a ales mapa
			{
				...
				else
				{
					...
					{
						SDL_RenderClear(renderer);
						TextureManager::Draw(gameover, { 0, 0, 1000, 800 }, { 0, 0, 1000, 800 });
						SDL_RenderPresent(renderer);
						replayflag = mouse({ 303,370,394,60 }) ? true : false;
			...
			{
				SDL_RenderClear(renderer);
				TextureManager::Draw(levels, { 0, 0, 1000, 800 }, { 0, 0, 1000, 800 });
				SDL_RenderPresent(renderer);
			}
		}
		else
		{
			SDL_RenderClear(renderer);
			TextureManager::Draw(chooseCharacter, { 0, 0, 1000, 800 }, { 0, 0, 1000, 800 });
